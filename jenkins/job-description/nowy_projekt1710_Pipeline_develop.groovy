...
def job_config = [
        job: [
                name: "nowy_projekt1710_Pipeline_develop"
        ],
        git: [
                branch: "feature-1666001222612"
        ]
]
...
def customLib = library identifier: 'nowy_projekt1710_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/nowy_projekt1710_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.nowy_projekt1710.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "nowy_projekt1710_Pipeline_develop", "feature-1666001222612")
                }
            }    
        }
        stage('Build') {
